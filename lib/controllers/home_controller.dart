  ...
  getCategories() async {
    ...
    await categoriesRef.get().then((allCategories) {
      for (int i = 0; i < allCategories.docs.length; i++) {
        categories.add(CategoryModel.fromJson(allCategories.docs[i].data()as Map<String, dynamic>));
      }
      loading.value = false;
  ...
  getProducts() async {
    ...
    await productsCollectionRef.get().then((allProducts) {
      for (int i = 0; i < allProducts.docs.length; i++) {
        ourAllProducts.add(ProductModel.fromJson(allProducts.docs[i].data()as Map<String, dynamic>));
      }
