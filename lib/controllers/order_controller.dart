  ...
  getUserOrders() async {
    ...
    await allOrdersRef
      ...
      for (int i = 0; i < orders.docs.length; i++) {
        if (OrderModel.fromJson(orders.docs[i].data()as Map<String, dynamic>).userId ==
            Get.find<ProfileController>().currentUser?.userId) {
          userOrders.add(OrderModel.fromJson(orders.docs[i].data()as Map<String, dynamic>));
        }
      }
    });
